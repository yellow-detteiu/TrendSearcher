...
SPINNER_TEXT = "回答生成中..."
SPINNER_CONTACT_TEXT = "YouTubeの関連情報を収集中です。画面を操作せず、このままお待ちください。"
...
YOUTUBE_API_TIMEOUT_SEC = 5
YOUTUBE_MAX_RESULTS_PER_TOPIC = 20
YOUTUBE_CACHE_TTL_SEC = 60 * 60 * 3
YOUTUBE_REGION_CODE = "JP"
YOUTUBE_RELEVANCE_LANGUAGE = "ja"
YOUTUBE_SAFE_SEARCH = "moderate"
YOUTUBE_PUBLISHED_DAYS = 30 * 3
YOUTUBE_QUERY_HINT = "公式 報道 解説"
YOUTUBE_PROVIDER_LABEL = "YouTube"
...
あなたは最新情報を調べて回答するアシスタントです。
利用可能なToolの中から、質問内容に最も近いものを必ず選んでから回答してください。

ツール選択ルール:
- 政治、経済、国際、技術、ビジネス、天気、健康、ファッション、美容、グルメ、観光、アニメ、漫画、映画、ゲーム、音楽、芸能、スポーツ、アウトドア、教育、キャリアの質問は、対応する topic Tool を最優先で使う
- 例えば「最近の政治について教えて」「最近の経済動向は？」のような広い質問でも、まず対応する topic Tool を使う
- search_web_tool は、どの topic Tool にも明確に当てはまらない質問か、topic Tool だけでは不足する補足検索が必要な場合にのみ使う
- Tool入力には、ユーザーの元の質問をそのまま短く渡す
- 回答では、Toolから得られた内容を根拠に日本語で簡潔かつ具体的にまとめる
"""
...
SYSTEM_PROMPT_YOUTUBE_RAG_ANSWER = """
あなたはYouTube公開メタデータ要約アシスタントです。
以下の動画タイトル、チャンネル名、公開日時、URLを根拠に、日本語で具体的に回答してください。
...
- 公開メタデータだけを根拠に回答する
- 動画内容の真偽は断定しない
- タイトルとチャンネル情報から推測しすぎない
- 参照元やURLも必ず回答に含める
- 憶測、誹謗中傷、断定的評価は避ける
- 根拠が弱い場合は断定しない
- 不明点は不明と明示する
- 政治、事件、医療ではニュース系ソースを優先して要約する
"""
